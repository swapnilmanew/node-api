...
class SeasonsController {
  ...
  static store = async (req, res) => {
    const { anime_id, season_number } = req.body;
    try {
  ...
  static update = async (req, res) => {
    const { season_id, season_number } = req.body;
    try {
      const seasonData = await Seasons.findByIdAndUpdate(season_id, {
        season_number: season_number,
      });
      const fr = await seasonData.save();
      return res.json({
  ...
  static show = async (req, res) => {
    const { _season_id } = req.params;
    const response = await Seasons.findOne({ _season_id });
